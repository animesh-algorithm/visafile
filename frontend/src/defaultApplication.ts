import type { Ds160Application } from "./types";

/** Schema-valid sample (CEAC wire values). Replace with real data before submit. */
export function createDefaultApplication(): Ds160Application {
  return {
    meta: {
      locationCode: "HYD",
      securityAnswer: "TestAnswer",
      allowSubmit: false,
    },
    personalInformation1: {
      surname: "TEST",
      givenNames: "APPLICANT",
      fullNameNativeAlphabetDoesNotApply: true,
      otherNamesUsed: "NO",
      telecodeNameUsed: "NO",
      sex: "M",
      maritalStatus: "S",
      dateOfBirth: { day: "01", month: "JAN", year: "1990" },
      cityOfBirth: "HYDERABAD",
      stateProvinceOfBirth: "TELANGANA",
      stateProvinceOfBirthDoesNotApply: false,
      countryRegionOfBirth: "IND",
    },
    personalInformation2: {
      nationality: "IND",
      hasOtherNationality: "NO",
      isPermanentResidentOfOtherCountry: "NO",
      nationalIdentificationNumberDoesNotApply: true,
      hasSSN: "NO",
      hasTaxpayerID: "NO",
    },
    travelInformation: {
      purposeOfTrip: "B",
      otherPurpose: "B1-B2",
      hasSpecificTravelPlans: "NO",
      arrivalDate: { day: "15", month: "12", year: "2026" },
      lengthOfStay: { duration: "14", unit: "D" },
      usAddress: {
        street: "123 Main Street",
        city: "New York",
        state: "NY",
        zipCode: "10001",
      },
      payingForTrip: "S",
    },
    travelCompanions: {
      otherPersonsTravelingWithYou: "NO",
    },
    previousUsTravel: {
      beenToUs: "NO",
      visaIssued: "NO",
      immigrantPetition: "NO",
    },
    addressPhone: {
      street: "123 TEST STREET",
      city: "HYDERABAD",
      state: "TELANGANA",
      postalCode: "500001",
      country: "IND",
      mailingSameAsHome: "YES",
      primaryPhone: "9876543210",
      workPhoneDoesNotApply: true,
      secondaryPhoneDoesNotApply: true,
      email: "test.applicant@example.com",
      additionalPhone: "NO",
      additionalEmail: "NO",
      socialMedia: "NO",
      socialMediaPlatform: "NONE",
    },
    passport: {
      passportType: "R",
      passportNumber: "X1234567",
      bookNumberDoesNotApply: true,
      issuedCountry: "IND",
      issuedCity: "HYDERABAD",
      issuedState: "TELANGANA",
      issuedInCountry: "IND",
      issuedDay: "01",
      issuedMonth: "01",
      issuedYear: "2020",
      expireDay: "01",
      expireMonth: "01",
      expireYear: "2030",
      lostStolen: "NO",
    },
    usContact: {
      surname: "CONTACT",
      givenNames: "TEST",
      organization: "TEST HOTEL",
      relationship: "C",
      street: "123 MAIN STREET",
      city: "NEW YORK",
      state: "NY",
      postalCode: "10001",
      phone: "2125550100",
      email: "us.contact@example.com",
    },
    family: {
      fatherSurname: "FATHER",
      fatherGivenNames: "TEST",
      fatherDobDay: "01",
      fatherDobMonth: "JAN",
      fatherDobYear: "1960",
      fatherLiveInUs: "NO",
      motherSurname: "MOTHER",
      motherGivenNames: "TEST",
      motherDobDay: "01",
      motherDobMonth: "JAN",
      motherDobYear: "1962",
      motherLiveInUs: "NO",
      immediateUsRelative: "NO",
      otherUsRelative: "NO",
    },
    workEducation: {
      occupation: "N",
      notEmployedExplain: "STUDENT",
    },
    previousWorkEducation: {
      previouslyEmployed: "NO",
      otherEducation: "NO",
    },
    additionalWorkEducation: {
      clanOrTribe: "NO",
      languages: ["ENGLISH"],
      countriesVisited: "NO",
      organization: "NO",
      specializedSkills: "NO",
      militaryService: "NO",
      insurgentOrg: "NO",
    },
    securityBackground: {
      answerRemainingYesNo: true,
      defaultYesNo: "NO",
    },
    signSubmit: {
      assistedByPreparer: "NO",
      passportNumber: null,
    },
  };
}
