...
const SESSION_FILE = resolve("data/session.json");
const PERSONAL_INFO_1_FIELDS_FILE = resolve(
  "data/personal-information-1.fields.json",
);
const PERSONAL_INFO_1_TEST_DATA_FILE = resolve(
  "fixtures/personal-information-1.test.json",
);
const PERSONAL_INFO_2_FIELDS_FILE = resolve(
  "data/personal-information-2.fields.json",
);
const PERSONAL_INFO_2_TEST_DATA_FILE = resolve(
  "fixtures/personal-information-2.test.json",
);
const TRAVEL_INFO_FIELDS_FILE = resolve(
  "data/travel-information.fields.json",
);
const TRAVEL_INFO_TEST_DATA_FILE = resolve(
  "fixtures/travel-information.test.json",
);
const TRAVEL_COMPANIONS_FIELDS_FILE = resolve(
  "data/travel-companions.fields.json",
);
const TRAVEL_COMPANIONS_YES_FIELDS_FILE = resolve(
  "data/travel-companions.yes.fields.json",
);
const TRAVEL_COMPANIONS_GROUP_YES_FIELDS_FILE = resolve(
  "data/travel-companions.group-yes.fields.json",
);
const TRAVEL_COMPANIONS_GROUP_NO_FIELDS_FILE = resolve(
  "data/travel-companions.group-no.fields.json",
);
const TRAVEL_COMPANIONS_TEST_DATA_FILE = resolve(
  "fixtures/travel-companions.test.json",
);
const PREVIOUS_US_TRAVEL_FIELDS_FILE = resolve(
  "data/previous-us-travel.fields.json",
);
const PREVIOUS_US_TRAVEL_TEST_DATA_FILE = resolve(
  "fixtures/previous-us-travel.test.json",
);
const ADDRESS_PHONE_FIELDS_FILE = resolve("data/address-phone.fields.json");
const ADDRESS_PHONE_TEST_DATA_FILE = resolve(
  "fixtures/address-phone.test.json",
);
const PASSPORT_FIELDS_FILE = resolve("data/passport.fields.json");
const PASSPORT_TEST_DATA_FILE = resolve("fixtures/passport.test.json");
const US_CONTACT_FIELDS_FILE = resolve("data/us-contact.fields.json");
const US_CONTACT_TEST_DATA_FILE = resolve("fixtures/us-contact.test.json");
const FAMILY_FIELDS_FILE = resolve("data/family.fields.json");
const FAMILY_TEST_DATA_FILE = resolve("fixtures/family.test.json");
const WORK_EDUCATION_FIELDS_FILE = resolve("data/work-education.fields.json");
const WORK_EDUCATION_TEST_DATA_FILE = resolve(
  "fixtures/work-education.test.json",
);
const SECURITY_BACKGROUND_FIELDS_FILE = resolve(
  "data/security-background.fields.json",
);
const SECURITY_BACKGROUND_TEST_DATA_FILE = resolve(
  "fixtures/security-background.test.json",
);
const SIGN_SUBMIT_FIELDS_FILE = resolve("data/sign-submit.fields.json");
const SIGN_SUBMIT_TEST_DATA_FILE = resolve("fixtures/sign-submit.test.json");
const CONFIRMATION_FIELDS_FILE = resolve("data/confirmation.fields.json");

const locationCode = process.env.DS160_LOCATION ?? "HYD";
...
const resumeApplication =
  process.env.DS160_RESUME === "true" || process.env.DS160_RESUME === "1";
const retrieveApplicationSelector =
  "#ctl00_SiteContentPlaceHolder_lnkRetrieve";
const CAPTCHA_FILE = resolve("data/captcha.txt");
const CAPTCHA_IMAGE_FILE = resolve("data/captcha.png");
 ...
 */
async function settleAfterPossiblePostback(page, label = "postback", options = {}) {
  const {
    allowEarlyExit = true,
...
function printTimingSummary() {
  ...
  for (const entry of sorted.slice(0, 25)) {
    ...
    log(`  ${formatMs(entry.elapsedMs).padStart(8)}  [${status}]  ${entry.label}`);
  }
  const totalTracked = stepTimings.reduce((sum, entry) => sum + entry.elapsedMs, 0);
  log(
...
async function saveCaptchaImage(page) {
  ...
  const src =
    dataUrl && typeof dataUrl === "object" ? dataUrl.src : null;
  const fetched = src
...
async function promptForCaptcha(page) {
  return timed("wait for CAPTCHA", async () => {
    ...
    await clearCaptchaFile();
    log(`CAPTCHA image: ${CAPTCHA_IMAGE_FILE}`);
    log(
      `Enter the code in ${CAPTCHA_FILE} (one line, then save) — open the PNG to read it`,
    );

    const deadline = Date.now() + 10 * 60_000;
    let polls = 0;
    while (Date.now() < deadline) {
      const fromFile = await readCaptchaFromFile();
      if (fromFile) {
        await clearCaptchaFile();
        log(`CAPTCHA read from file after ${polls} poll(s)`);
        return fromFile;
      }
      polls += 1;
      if (polls === 1 || polls % 15 === 0) {
        log(
          `still waiting for CAPTCHA… open ${CAPTCHA_IMAGE_FILE} → type into ${CAPTCHA_FILE}`,
        );
      }
      await sleep(1000, "CAPTCHA file poll");
    }

    throw new Error(
      `Timed out waiting for CAPTCHA. Open ${CAPTCHA_IMAGE_FILE}, edit ${CAPTCHA_FILE}, or set DS160_CAPTCHA.`,
    );
  });
}
  ...
  const selected = await page.evaluate(
    (partial, requestedValue) => {
      ...
      const candidates = [
        ...
        (candidate) =>
          candidate.id.includes(partial) ||
          candidate.name.includes(partial),
      );
      const target = candidates.find(
        (candidate) =>
...
async function getPageValidationErrors(page) {
  return page.evaluate(() => {
    ...
    const looksLikeValidationMessage = (message) => {
      ...
      if (/Select Tooltip Language|Contact Us Help|COMPLETE REVIEW/i.test(message)) {
        return false;
      }
      return (
  ...
  return timed("clickNext", async () => {
    ...
    const navigationPromise = page
      ...
      .catch((error) => {
        ...
        if (/detached|LifecycleWatcher|context was destroyed/i.test(error.message)) {
    ...
    const errors = await logPageValidationErrors(page, "After clicking Next");
    if (errors.length > 0) {
      throw new Error(
  ...
  return page.evaluate((snippet) => {
    ...
    for (const cell of document.querySelectorAll("td, th, li, div, span")) {
      ...
      const candidates = [
        ...
        ...(row.nextElementSibling?.querySelectorAll("input, select, textarea") ??
          []),
      ].filter(isFillable);
  ...
  await page.evaluate(
    (controlTarget, fieldValue, fieldKind) => {
      ...
      if (typeof controlTarget === "string") {
        ...
        element = document.querySelector(`[name="${CSS.escape(controlTarget.name)}"]`);
      }
...
async function resolveUsStayAddressControls(page) {
  ...
  const city =
    byIdPattern.city || (await findControlIdByLabel(page, "City"));
  const state =
    byIdPattern.state || (await findControlIdByLabel(page, "State"));
  const zip =
    byIdPattern.zip ||
    (await findControlIdByLabel(page, "ZIP Code"));
...
async function fillTravelInformation(page) {
  const data = JSON.parse(
    await readFile(TRAVEL_INFO_TEST_DATA_FILE, "utf8"),
  );
  ...
  if (data.usAddress) {
    ...
    if (!addressIds.street || !addressIds.city || !addressIds.state) {
      ...
      const blockIds = await page.evaluate(() => {
        const heading = [...document.querySelectorAll("td, th, span, div, b")].find(
          (element) => {
            const text = element.textContent?.replace(/\s+/g, " ").trim() ?? "";
            return (
        ...
        for (let i = 0; i < 6 && root; i += 1) {
          const fillable = [...root.querySelectorAll("input, select, textarea")]
            .filter(
              (element) =>
                !["hidden", "image", "submit", "button", "radio", "checkbox"]
          ...
          const selects = fillable.filter((element) => element.tagName === "SELECT");
      ...
      if (blockIds) {
        addressIds = {
          ...
          zip: blockIds.zip || (blockIds.zipName ? { name: blockIds.zipName } : null),
        };
      }
    }
...
async function fillTravelCompanions(page) {
  ...
  if (probeConditionals) {
    ...
    if (await pageHasPartialId(page, "rblGroupTravel")) {
      ...
      await maybeSaveFieldInventory(page, TRAVEL_COMPANIONS_GROUP_YES_FIELDS_FILE);

      await chooseRadioWithPostback(page, "rblGroupTravel", "NO");
      await maybeSaveFieldInventory(page, TRAVEL_COMPANIONS_GROUP_NO_FIELDS_FILE);
    }
  }
  ...
  if (data.otherPersonsTravelingWithYou === "YES") {
    ...
    if (asGroup === "YES") {
      const groupNamePartial =
        (await pageHasPartialId(page, "tbxGroupName"))
          ? "tbxGroupName"
      ...
      const relationshipPartial = (
        await pageHasPartialId(page, "ddlTOP_RELATIONSHIP")
      )
        ? "ddlTOP_RELATIONSHIP"
...
async function retrieveExistingApplication(page, session) {
  ...
  const retrieveSelector =
    (await page.$(retrieveApplicationSelector))
...
async function fillAddressAndPhone(page) {
  ...
  for (const [partial, value] of expected) {
    ...
    if ((actual || "").trim().toUpperCase() !== String(value).trim().toUpperCase()) {
...
async function isConfirmationPage(page) {
  return page.evaluate(() => {
    ...
    return (
      ...
      hasBarcodeImage ||
      hasPrint ||
      /has been submitted|thank you for submitting|confirmation page/i.test(text)
    );
  });
}
...
async function clickControlByText(page, pattern) {
  ...
  const clicked = await page.evaluate((source) => {
    ...
    const target = candidates.find((candidate) =>
      re.test(`${candidate.value || ""} ${candidate.textContent || ""} ${candidate.id}`),
    );
    if (!target) {
      return null;
    }
...
async function handleConfirmationPage(page) {
  ...
  const pdfPath = resolve("data/ds160-confirmation.pdf");
  const pngPath = resolve("data/ds160-confirmation.png");
  ...
  await writeFile(
    resolve("data/confirmation.json"),
    `${JSON.stringify(payload, null, 2)}\n`,
    { mode: 0o600 },
  );
  await chmod(resolve("data/confirmation.json"), 0o600);
...
async function goToConfirmationFromSignPage(page) {
  ...
  const clicked = await clickControlByText(page, /next:\s*confirmation|confirmation/i);
  if (!clicked) {
  ...
  const clicked = await page.evaluate(() => {
    const button =
      ...
      [...document.querySelectorAll('input[type="submit"], button')].find(
        (candidate) =>
          /sign and submit/i.test(candidate.value || candidate.textContent || ""),
      );
    if (!button) {
      return false;
    }
...
async function fillSignAndSubmit(page) {
  ...
  if (!hasSignButton) {
    console.log(
      'No Sign button on page; clicking through to Confirmation...',
    );
  ...
  if (await pageHasPartialId(page, "rblPREP_IND")) {
    ...
    if (data.assistedByPreparer === "YES" && data.preparer) {
      for (const [partial, value] of Object.entries(data.preparer.texts || {})) {
...
async function fillWorkEducation3(page) {
  const fieldsFile = resolve("data/work-education-3.fields.json");
  const testFile = resolve("fixtures/work-education-3.test.json");
  const defaults = {
    clanOrTribe: "NO",
    languages: ["ENGLISH"],
    countriesVisited: "NO",
...
async function fillPreviousWorkEducation(page) {
  const fieldsFile = resolve("data/work-education-2.fields.json");
  const testFile = resolve("fixtures/work-education-2.test.json");
  const defaults = {
...
async function fillFamilyRelatives(page) {
  ...
  await fillTextByPartialId(page, "tbxFATHER_GIVEN_NAME", data.fatherGivenNames);
  ...
  await fillTextByPartialId(page, "tbxMOTHER_GIVEN_NAME", data.motherGivenNames);
  ...
  if (!streetPartial) {
    ...
    throw new Error(
      `U.S. Contact address fields did not appear after relationship. Controls:\n  - ${controls
        .map((control) => `${control.id || control.name} | ${control.label || "-"}`)
        .join("\n  - ")}`,
    );
  }
...
async function fillPreviousUsTravel(page) {
  ...
  try {
    data = {
      ...data,
      ...JSON.parse(await readFile(PREVIOUS_US_TRAVEL_TEST_DATA_FILE, "utf8")),
    };
  } catch {
    await mkdir(resolve("fixtures"), { recursive: true });
    await writeFile(
...
async function fillGenericMostlyNoPage(page, fieldsFile, testDataFile, defaults) {
  let data = { ...defaults };
  try {
    data = {
      ...data,
  ...
  if (data.checkboxes && typeof data.checkboxes === "object") {
    log(`  applying ${Object.keys(data.checkboxes).length} checkbox mapping(s)`);
...
async function continueFromCurrentPage(page) {
  ...
  for (let step = 0; step < 25; step += 1) {
    ...
    if (
      ...
      if (!/SignCertify|E-Signature and Certification/i.test(`${node} ${title}`)) {
        await timed("handleConfirmationPage", () => handleConfirmationPage(page));
    ...
    if (/Review/i.test(`${node} ${title}`) || /Photo|UploadPhoto/i.test(`${node}`)) {
      await maybeSaveFieldInventory(
        page,
        resolve(`data/${(node || "review").toLowerCase()}.fields.json`),
      );
    ...
    if (/Security|Background/i.test(`${node} ${url}${title}`)) {
      ...
      await timed(`fill Security (${suffix})`, async () => {
        await fillGenericMostlyNoPage(
          page,
          resolve(`data/security-background-${suffix}.fields.json`),
    ...
    log(`Unknown page captured (${controls.length} controls). Attempting Next...`);
    try {
      await clickNext(page);
      log(`page wall time ${formatMs(Date.now() - pageStarted)}`);
    } catch (error) {
